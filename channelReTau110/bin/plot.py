...
def parser():
    ...
    p.add_argument('-E','--errorBarLineWidth', help='Error bar line width', type=float, default=1)
    p.add_argument('-C','--capSize', help='Cap size', type=float, default=3)
    ...
    p.add_argument('--legendFontSize', help='Legend font size', type=int, default=6)
    ...
    p.add_argument('--tickFontSize', help='Tick font size', type=int, default=10)
    p.add_argument('--solverTickFontSize', help='Solver tick font size', type=int, default=10)
    p.add_argument('--topFractionSolver'
                   , help='Top fraction in solver-* plot', type=float, default=0.95)
    p.add_argument('--bottomFractionSolver'
                   , help='Bottom fraction in solver-* plot', type=float, default=0.35)
    p.add_argument('--rightFractionSolver'
                   , help='Right fraction in solver-* plot', type=float, default=0.5)
    p.add_argument('--leftFractionSolver'
                   , help='Left fraction in solver-* plot', type=float, default=0.1)
    p.add_argument('--topFractionMpi'
                   , help='Top fraction in mpi-* plot', type=float, default=0.95)
    p.add_argument('--bottomFractionMpi'
                   , help='Bottom fraction in mpi-* plot', type=float, default=0.15)
    p.add_argument('--rightFractionMpi'
                   , help='Right fraction in solver-* plot', type=float, default=0.5)
    p.add_argument('--leftFractionMpi'
                   , help='Left fraction in solver-* plot', type=float, default=0.1)
...
def plotMpi(args,plotfile,mpi,ylabel):
    plt.subplots_adjust(
        top=args.topFractionMpi
        ,bottom=args.bottomFractionMpi
        ,right=args.rightFractionMpi
        ,left=args.leftFractionMpi
        )
    plt.grid()
    ...
    ax=plt.gca()
    ax.xaxis.set_minor_locator(MultipleLocator(1e+30))
...
def plotEnd(plotfile):
    plt.legend(bbox_to_anchor=(1.05, 1), loc='upper left', borderaxespad=0, prop={'size':args.legendFontSize})
    ...
    for decomposeParDict in pd.unique(data['decomposeParDict']):
        ...
        for bFNI in range(len(args.batchFileNameList)):
            ...
            if args.maxNumberOfSampling>1:
                plt.errorbar(x
                             , ExecutionTimeAve
                             , yerr=ExecutionTimeSTD
                             , label=labelName
                             , linewidth=args.lineWidth
                             , elinewidth=args.errorBarLineWidth
                             , capsize=args.capSize
                             )
            else:
                plt.plot(x, ExecutionTimeAve, label=labelName, linewidth=args.lineWidth)

        plt.subplots_adjust(top=args.topFractionSolver
                            ,bottom=args.bottomFractionSolver
                            ,right=args.rightFractionSolver
                            ,left=args.leftFractionSolver)
    ...
    for solver in pd.unique(data['fvSolution']):
        for method in pd.unique(data['method']):
            ...
            for bFNI in range(len(args.batchFileNameList)):
                ...
                if args.maxNumberOfSampling>1:
                    plt.errorbar(mpi
                                 , ExecutionTimeFirstStepAve
                                 , yerr=ExecutionTimeFirstStepSTD
                                 , label=labelName
                                 , linewidth=args.errorBarLineWidth
                                 , elinewidth=args.errorBarLineWidth
                                 , capsize=args.capSize
                                 )
                else:
    ...
    for solver in pd.unique(data['fvSolution']):
        for method in pd.unique(data['method']):
            ...
            for bFNI in range(len(args.batchFileNameList)):
                ...
                if args.maxNumberOfSampling>1:
                    plt.errorbar(mpi
                                 , numberOfStepsPerHourAve
                                 , yerr=numberOfStepsPerHourSTD
                                 , label=labelName
                                 , linewidth=args.lineWidth
                                 , elinewidth=args.errorBarLineWidth
                                 , capsize=args.capSize)
